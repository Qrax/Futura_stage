...
void setup_i2s_adc() {
  ...
  esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTENUATION, ADC_BIT_WIDTH, 0, adc_chars);
...
void trigger_and_measure() {
  ...
  if (bytes_read > 0) {
    ...
    for (int i = 0; i < I2S_DMA_BUF_LEN; i++) {
      ...
      uint32_t voltage_mv = esp_adc_cal_raw_to_voltage(adc_capture_buffer[i], adc_chars);
